...
void afficher_aide(const char* nom) {
    std::cout << "Usage : " << nom << " [options]\n"
              << "Options CLI (resume texte) :\n"
              << "  --horizon <heures>            Duree de simulation (defaut 8)\n"
              << "  --ors <nb>                    Nombre de salles d'operation (defaut 2)\n"
              << "  --recovery-beds <nb>          Nombre de lits de reveil (defaut 3)\n"
              << "  --elective-count <nb>         Nombre de patients programmes (defaut 10)\n"
              << "  --elective-window <heures>    Fenetre d'arrivee des programmes (defaut 6)\n"
              << "  --urgent-rate <par_heure>     Frequence des urgences (patients/heure, defaut 2)\n"
              << "  --mean-surgery-elective <m>   Duree moy. chirurgie programmee (minutes)\n"
              << "  --mean-surgery-urgent <m>     Duree moy. chirurgie urgente (minutes)\n"
              << "  --mean-recovery <m>           Duree moy. de reveil (minutes)\n"
              << "  --policy <fifo|priority|priorite|balanced|equilibre> Politique d'ordonnancement (defaut priority)\n"
...
bool parse_int(const std::string& valeur, int& out) {
    try {
        ...
        if (idx != valeur.size() || parsed < 0) return false;
...
SchedulingPolicy parse_policy(const std::string& value) {
    if (value == "fifo") return SchedulingPolicy::Fifo;
    if (value == "priority" || value == "priorite") return SchedulingPolicy::PriorityFirst;
    if (value == "balanced" || value == "equilibre") return SchedulingPolicy::Balanced;
...
std::string rendre_rapport(const SimulationConfig& config,
    ...
    os << "Attente moyenne vers reveil : " << report.average_wait_to_recovery
       << " min\n";
    os << "Temps moyen dans le systeme : "
       << report.average_total_time_in_system << " min\n";
    os << "Utilisation blocs : " << report.operating_room_utilization * 100.0
       << "% | Utilisation reveil : "
       << report.recovery_bed_utilization * 100.0 << "%\n";
